...
6_delete_view_edit_impl.mp4
    ...
    в doGet    
    вводим параметр action
    
    с 7 появились стринги в свитчах

    нужно переправиить в jsp

    0349

    про форвард и редирект
    редирект - когда мы отдаем URL страницы в браузер и 
    переходим по нему - форвардимся

    прикручиваем в jsp action delete, view, edit

    0546

    добавление и редактирование будет через одну и ту же jsp
...
IDEA
    ...
    разбор средства интеграции хрома
      сеть
      запрос на delete

    первый запрос пришел в сервлет

    делается редирект, нам в браузер приходит ответ (респонс) 
    302 Found

    в респонс хедерах нам приходит 
    локейшн, куда делать редирект

    след строка запрос с новым локейшном resume

    по реквесту с resume мы снова попадаем в наш сервлет

    снова берем все резюме, без того, который мы удалили

    отличие от форварда
      при форварде - мы остаемся на сервере 
      при редиректе - мы возвращаем код 302 и делаем новый запрос по новому локейшену

      можно делать редирект для того, чтобы сбрасывалась URL

    1050

    добавляем view.jsp, edit.jsp для отображения и редактирования

    1230

    можем задавать атрибут scope req из - резюме будет из 
    реквеста браться

    1515
    
    по умолчанию скоуп - внутри страницы

    смешиваем логику отображения в модель
...
1_HW16.mp4 - Shortcut
    ...
    если более сложное что-то, то уже другое, более сложное
    нужно использовать

    JDBC, JSP сложно становится рефакторить и поддерживать

    0154

    нужно еще редактирование и отображение секций

    0958

    получилось сделать секции
    ...
    требуется js
      по вставке мы должны в документ html вставить 
      все теги, что не просто

      разбирается на 8-ом занятии

    просто добавляем пустое

    2141

    переходим к сервлету
    будем сохранять через doPost и отображать

    2500
    
    пустая орг
    emptyFirstOrganizations
